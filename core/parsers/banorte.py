...
class BanorteParser(BaseBankParser):
    ...
    def _parse_outgoing_transfer(self, text, email_id) -> Transaction | None:
        ...
        amount_match = re.search(r'Importe: </td>\s*<td nowrap=\"nowrap\">\s*\$([\d,]+\.\d{2}) MN\s*</td>', text)
        
        if amount_match:
            amount = amount = float(amount_match.group(1).replace(',', ''))
            
        description_match = re.search(r'Operación: </td>\s*<td align=\"left\" nowrap=\"nowrap\">\s*(.*?)\s*</td>', text)
        if description_match:
            description = description_match.group(1)
        
        time_match = re.search(r'Hora de Operación: </td>\s*<td nowrap="nowrap">\s*(\d{2}:\d{2}:\d{2} horas)\s*</td>', text, re.DOTALL | re.IGNORECASE)
        if time_match:
            time = time_match.group(1).replace('horas', '').strip()

        date_match = re.search(r"Fecha de Operación: </td>\s*<td nowrap=\"nowrap\">\s*(\d{1,2}/[A-Za-z]{3}/\d{4})\s*</td>", text)
        if date_match:
